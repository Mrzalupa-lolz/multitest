...
```bash
bash <(curl -sL https://raw.githubusercontent.com/Mrzalupa-lolz/multitest/master/multitest.sh)
```

**Установка как команда `multitest`:**

```bash
curl -sL https://raw.githubusercontent.com/Mrzalupa-lolz/multitest/master/multitest.sh -o /usr/local/bin/multitest && chmod +x /usr/local/bin/multitest && echo "Установлено! Запуск: multitest"
```
